...
void LCD_Draw_Rectangle(uint16_t Xpos, uint16_t Ypos, uint16_t Width, uint16_t Height, uint16_t Color) {
    for (uint16_t x = Xpos; x < Xpos + Width; x++) {
        LCD_Draw_Pixel(x, Ypos, Color);
        LCD_Draw_Pixel(x, Ypos + Height - 1, Color);
    }
    for (uint16_t y = Ypos; y < Ypos + Height; y++) {
        LCD_Draw_Pixel(Xpos, y, Color);
        LCD_Draw_Pixel(Xpos + Width - 1, y, Color);
    }
}

void LCD_Draw_FillRectangle(uint16_t Xpos, uint16_t Ypos, uint16_t Width, uint16_t Height, uint16_t Color) {
    for (uint16_t y = Ypos; y < Ypos + Height; y++)
        for (uint16_t x = Xpos; x < Xpos + Width; x++)
...
void LCD_ShowMenu(void) {
	...
	for (uint16_t x = 0; x < LCD_PIXEL_WIDTH; x++) {
		LCD_Draw_Pixel(x, LCD_PIXEL_HEIGHT/2, LCD_COLOR_BLACK);
	}
	
	LCD_SetTextColor(LCD_COLOR_BLACK);
	LCD_SetFont(&Font16x24);
	LCD_DisplayString(LCD_PIXEL_WIDTH/2 - 50, 100, "One Player");
	LCD_DisplayString(LCD_PIXEL_WIDTH/2 - 60, LCD_PIXEL_HEIGHT/2 + 50, "Two Players");
}
